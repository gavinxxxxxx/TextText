 ...
 */
public class TestManager extends BaseManager implements DataLayer.SourceService {

    @Override
    public Observable<Book> search(Source source, String query) {
        return getApi().get(source.queryUrl(query))
    ...
    public Observable<List<Chapter>> directory(Source source, String id) {
        return getApi().get(source.directoryUrl(id))
                .map(ResponseBody::byteStream)
                .map(this::read)
                .map(Jsoup::parse)
                ...
                .flatMap(file -> {
                    if (!file.exists() || file.isDirectory()) {
                        Chapter chapter = getDaoSession().getChapterDao()
                                ...
                                .unique();
                        return getApi().get(source.chapterUrl(chapter))
                                .map(ResponseBody::byteStream)
                                .map(this::read)
