 ...
 */
public class ITextView2 extends View {
    ...
    public ITextView2(Context context) {
        this(context, null);
    }

    public ITextView2(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        if (attrs != null) {
            int index = -1;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mSingleLine) {
            setMeasuredDimension(widthSize, heightMode == MeasureSpec.EXACTLY ? heightSize : (int) Math.ceil(mTextHeight));
            return;
        }
    ...
    private void draw1(Canvas canvas) {
        ...
        float lineWidth = 0;
        float baseY = (int) (mTextHeight / 2 - mBPaint.descent() / 2 - mBPaint.ascent() / 2);
        ...
        for (int i = 0; i < mTexts.size(); i++) {
            ...
            if (lineWidth + hSpace + childWidth <= getWidth() - getPaddingLeft() - getPaddingRight()) { // 加上空格和内容放得下
                ...
                childWidth = getWidth() - getPaddingLeft() - getPaddingRight() - lineWidth - hSpace;
                hasNext = false;
            }
            ...
            if (text.type == IText.TYPE_STRIKETHROUGH) {
                canvas.drawRect(left, mTextHeight / 2 - 2.5f, left + w, mTextHeight / 2 + 2.5f, mRPaint);
            }
